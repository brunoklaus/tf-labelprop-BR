...
import numpy as np
from tf_labelprop.experiment.experiments import ExperimentRun, TIME_HOOKS
from tf_labelprop.experiment.hooks.hook_skeleton import GSSLHook
from tf_labelprop.experiment.hooks.ldst_filterstats_hook import LDSTFilterStatsHook
from tf_labelprop.experiment.prefixes import FILTER_PREFIX
from tf_labelprop.experiment.selector import Hook
...
class FilterLDST(EmptySpecification):
    ...
    def run(self,cfg):
        res = ExperimentRun(cfg).run(hook_list=[Hook.LDST_STATS_HOOK])
        res.update(cfg)
